...
# Locate backend/frontend directories (script is expected to be in '正式搭建' folder)
$ScriptDir    = Split-Path -Parent $MyInvocation.MyCommand.Path
$BackendPath  = Join-Path $ScriptDir "backend"
$FrontendPath = Join-Path $ScriptDir "frontend"

if (-not (Test-Path $BackendPath) -or -not (Test-Path $FrontendPath)) {
    Write-Host "ERROR: backend or frontend folder not found. Please keep run-dev.ps1 in the same folder as 'backend' and 'frontend'." -ForegroundColor Red
    exit 1
}
